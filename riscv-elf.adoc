...
NOTE: Note, that although the generated code is position independent, but
it's not suitable for building ELF shared libraries, please use medium position
independent code model for such scenario.

=== Medium position independent code model

This model is like the `medium`, but must get symbol address from global offset
table (GOT) for non-local symbol.
...
1:  auipc a0, %pcrel_hi(symbol)
    ...
    # Calculate address of local symbol
2:  auipc a0, %pcrel_hi(symbol)
    addi  a0, a0, %pcrel_lo(2b)

    # Calculate address
3:  auipc a0, %got_pcrel_hi(symbol)
    lw  a0, a0, %pcrel_lo(3b)       # ld for RV64
----
